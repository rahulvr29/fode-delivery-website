...
const Header = () => {
  ...
  {/* tablet and desktop work */}
      <div className='hidden md:flex w-full h-full items-center justify-between'>
        ...
        <motion.ul 
            ...
            <li className='text-base text-textColor hover:text-red-500  duration-100 transition-all ease-in-out cursor-pointer'>Home</li>
            <li className='text-base text-textColor hover:text-red-500 duration-100 transition-all ease-in-out cursor-pointer'>Menu</li>
            <li className='text-base text-textColor hover:text-red-500 duration-100 transition-all ease-in-out cursor-pointer '>About Us</li>
            <li className='text-base text-textColor hover:text-red-500 duration-100 transition-all ease-in-out cursor-pointer '>Service</li>
          </motion.ul>

          <motion.div 
          whileTap={{ scale: 0.8 }}
          className='relative flex items-center justify-center' onClick={showCart}>
            <MdShoppingBasket  className='text-textColor text-2xl ml-8 mx-9 cursor-pointer'/>
            {cartItems && cartItems.length > 0 && (
              <div className=' absolute -top-1.5 -right-3 w-5 h-5 rounded-full mx-9  bg-cartNumBg flex items-center justify-center'>
            ...
            <motion.img 
            whileTap={{ scale: 0.7 }}
            src={ user ? user.photoURL : Avatar } className='w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl -mx-3 cursor-pointer rounded-full' alt="userprofile" onClick={login} />
  ...
  {/* mobile work */}
      ...
      <div className='flex justify-between md:hidden w-full h-full relative'>
          <Link to={'/'} className='flex items-center gap-2'>
              <img src={Logo} className='logo' alt="logo" />
          </Link>
          ...
          <motion.div 
              ...
              className='relative flex items-center '>
                <MdShoppingBasket  className='text-textColor text-2xl ml-12 mx-9 cursor-pointer' onClick={showCart}/>
                      {cartItems && cartItems.length > 0 &&(
                      <div className=' absolute -top-0 -right-3 w-5 h-5 rounded-full mx-9  bg-cartNumBg flex items-center justify-center'>
                        <p className='text-xs text-white font-semibold'>{cartItems.length}</p>
                      </div>
                      )}
          </motion.div>
